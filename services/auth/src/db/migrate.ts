 ...
 * in `auth.schema_migrations` so each runs at most once. Designed for `npm run migrate` and for the
 * service to self-migrate on boot in local/dev.
 */
...
export async function runMigrations(pool: pg.Pool): Promise<string[]> {
  ...
  for (const filename of entries) {
    const { rows } = await pool.query(
      'SELECT 1 FROM auth.schema_migrations WHERE filename = $1',
      [filename],
    );
    ...
    try {
      ...
      await client.query('INSERT INTO auth.schema_migrations (filename) VALUES ($1)', [filename]);
