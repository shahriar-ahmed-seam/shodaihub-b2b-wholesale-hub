 ...
 * in `payment.schema_migrations` so each runs at most once. Designed for `npm run migrate` and for
 * the service to self-migrate on boot in local/dev.
 */
...
export async function runMigrations(pool: pg.Pool): Promise<string[]> {
  await pool.query('CREATE SCHEMA IF NOT EXISTS payment');
  await pool.query(`
  ...
  for (const filename of entries) {
    const { rows } = await pool.query(
      'SELECT 1 FROM payment.schema_migrations WHERE filename = $1',
      [filename],
    );
    ...
    try {
      ...
      await client.query('INSERT INTO payment.schema_migrations (filename) VALUES ($1)', [filename]);
